            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form method="GET" action="{{ route('freelancers.index') }}"
                    ...
                    <div>
                        ...
                        <x-text-input id="keyword" name="keyword" type="text" class="mt-1 block w-56"
                            :value="$keyword" placeholder="氏名・フリガナ・スキル" />
                    </div>
                    <div>
                        <x-input-label for="partner_company_id" :value="__('所属')" />
                        <select id="partner_company_id" name="partner_company_id"
                            class="mt-1 block w-52 border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                            <option value="">{{ __('すべて') }}</option>
                            <option value="internal" @selected($partnerCompanyId === 'internal')>{{ __('社内直登録') }}</option>
                            @foreach ($companies as $company)
                                <option value="{{ $company->id }}"
                                    @selected((string) $partnerCompanyId === (string) $company->id)>{{ $company->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select id="status" name="status"
                            class="mt-1 block w-36 border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                            <option value="">{{ __('すべて') }}</option>
                    ...
                    @if (filled($keyword) || filled($status) || filled($partnerCompanyId))
                        <a href="{{ route('freelancers.index') }}" class="text-sm text-gray-600 hover:text-gray-900">{{ __('条件をクリア') }}</a>
                    @endif
                </form>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead>
                            <tr>
                                ...
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">{{ __('所属') }}</th>
                            ...
                            @forelse ($freelancers as $freelancer)
                                <tr>
                                    ...
                                    <td class="px-4 py-2 text-sm">
                                        @if ($freelancer->partnerCompany)
                                            <span class="text-gray-700">{{ $freelancer->partnerCompany->name }}</span>
                                        @else
                                            <span class="text-gray-400">{{ __('社内直登録') }}</span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-4 py-2 text-right space-x-2 whitespace-nowrap">
                                        <a href="{{ route('freelancers.edit', $freelancer) }}" class="text-sm text-gray-600 hover:text-gray-900">{{ __('編集') }}</a>
                                        <form action="{{ route('freelancers.destroy', $freelancer) }}" method="POST" class="inline"
